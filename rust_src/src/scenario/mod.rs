...
pub fn scenario_plan(
    ...
    // Scale the route-following params to the longest straight stretch the
    // boat will sail (measured before any gate sub-division, which only
    // adds collinear points). The planner's input defaults — tight XTE
    // lookahead, short minimum tack — suit close-quarters maneuvering, but
    // on a long close-hauled passage they make the boat pinch to the rhumb
    // line and chatter tacks at the no-go boundary, so it never gets
    // across. On a long leg, loosen the lookahead (foot for speed instead
    // of clawing back to the line) and raise the minimum tack (commit to
    // one or two long tacks). Short routes keep their base values via the
    // lower clamp. A/B runs: hold-line stalls 8 km short; long-tack
    // finishes in 31% less distance.
    let longest_leg = pts
        .windows(2)
        .map(|w| (w[1].0 - w[0].0).hypot(w[1].1 - w[0].1))
        .fold(0.0_f64, f64::max);
    let xte_lookahead =
        (0.10 * longest_leg).clamp(route.xte_lookahead, route.xte_lookahead.max(3000.0));
...
fn write_planned_route(
    ...
    writeln!(s, "xte_lookahead: {:.0}", xte_lookahead).ok();
    ...
    if xte_lookahead > base.xte_lookahead || min_tack_s > base.min_tack_duration_s {
        writeln!(
            s,
            "# Following params scaled up for long legs: foot for speed, commit to long tacks."
        )
        .ok();
    }
