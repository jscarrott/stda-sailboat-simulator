#!/usr/bin/env bash
# Sweep the pond triangle under several wind conditions.
# Wind specified in compass terms; converted to math direction (TOWARD).
#
...
for label in "${!DIRS[@]}"; do
    ...
    for spd in $SPEEDS; do
        ...
        echo "--- wind FROM $label @ ${spd} m/s (math dir ${deg}°) [$SOLVER] -> $out"
        ./target/release/sailboat_sim \
            --config "$CONFIG" --scenario route \
            --route "$ROUTE" \
            --wind-deg "$deg" --wind-speed "$spd" \
            --solver "$SOLVER" \
